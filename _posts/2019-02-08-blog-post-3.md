...
2. The Domain Layer: is the most INNER part of the onion and it contains Entities, Use cases & Repository Interfaces. Use cases combine data from 1 or multiple Repository Interfaces.. In the following example, we have a ```UseCase``` class that describes what the business needs.
``` kotlin
...
> many people think about use-case is useless part as it only calls the repository but from my point of view it's very important part of the architecture , lets take an example if you have a business rule says once you click the button you need to **add user to the queue** and you know that you cannot add the user without **creating the queue first** and here the benefit of the use-case you have to include only one rule which is business rule **addUserToQueue** and in this usecase you have to create the queue first then add the user to the queue so if you read the use-case you don't need to know that you have to create queue first all you want to know that their is a rule somewhere says when click on this button add the user to the queue.
